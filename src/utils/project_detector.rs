...
async fn detect_igra_project() -> Result<Option<ProjectConfig>> {
    ...
    let compose_content = tokio::fs::read_to_string(&compose_file).await
...
async fn detect_kasplex_project() -> Result<Option<ProjectConfig>> {
    ...
    let kasplex_root = PathBuf::from(std::env::var("HOME")?)
        .join("kasplex")
        .join("evm-l2-nodes-docker");
    ...
    let testnet_env = kasplex_root.join("docker-compose").join("envs").join("syncer.testnet.env");
    let mainnet_env = kasplex_root.join("docker-compose").join("envs").join("syncer.mainnet.env");
...
async fn detect_other_compose_projects() -> Result<Vec<ProjectConfig>> {
    ...
    for container in containers {
        if let Some(labels) = container.labels {
            ...
            if let (Some(name), Some(config), Some(workdir)) = (project_name, config_files, working_dir) {
                // Skip IGRA and Kasplex as they're detected separately
                if name == "igra-orchestra-testnet" || name == "igra-orchestra-mainnet" || name == "docker" {
                    continue;
                }
...
mod tests {
    ...
    async fn test_detect_projects() {
        let projects = detect_projects().await.unwrap();
        // Should find at least IGRA or Kasplex or other projects
