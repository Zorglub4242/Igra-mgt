...
use crate::core::{
    SystemServiceManager, CategoryManager, UserManager, SecurityManager, AuditLogger,
};
...
pub fn create_router(enable_cors: bool) -> Router {
    ...
    let user_manager = UserManager::new(config_dir.clone())
        .expect("Failed to initialize UserManager");

    let security_manager = Arc::new(RwLock::new(
        SecurityManager::new(config_dir.clone())
            .expect("Failed to initialize SecurityManager")
    ));

    let audit_logger = Arc::new(RwLock::new(
        AuditLogger::new(config_dir.clone())
            .expect("Failed to initialize AuditLogger")
    ));
    ...
    let public_routes = Router::new()
        ...
        .route("/api/auth/session", get(auth_handlers::session_info_handler))
    ...
    let protected_routes = Router::new()
        ...
        // System services
        .route("/api/system-services", get(system_service_handlers::list_system_services))
        .route("/api/system-services/available", get(system_service_handlers::list_available_services))
        .route("/api/system-services/{name}/details", get(system_service_handlers::get_service_details))
        .route("/api/system-services/{name}/logs", get(system_service_handlers::get_service_logs))
        .route("/api/system-services/{name}/start", post(system_service_handlers::start_service))
        .route("/api/system-services/{name}/stop", post(system_service_handlers::stop_service))
        .route("/api/system-services/{name}/restart", post(system_service_handlers::restart_service))
        .route("/api/system-services/{name}/enable", post(system_service_handlers::enable_service))
        .route("/api/system-services/{name}/disable", post(system_service_handlers::disable_service))

        // Categories
        .route("/api/categories", get(system_service_handlers::list_categories))
        .route("/api/categories/{id}", get(system_service_handlers::get_category))
        .route("/api/categories", post(system_service_handlers::create_category))
        .route("/api/categories/{id}", put(system_service_handlers::update_category))
        .route("/api/categories/{id}", delete(system_service_handlers::delete_category))
        .route("/api/categories/{id}/services", post(system_service_handlers::add_service_to_category))

        // Tracked services
        .route("/api/tracked-services", get(system_service_handlers::get_tracked_services))
        .route("/api/tracked-services/{name}", put(system_service_handlers::update_tracked_service))
        .route("/api/tracked-services/{name}", delete(system_service_handlers::remove_tracked_service))
        ...
        .route("/api/users/{username}/roles", put(handlers::update_user_roles))
