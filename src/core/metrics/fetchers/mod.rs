...
impl AnyFetcher {
    pub async fn fetch_metric(&self, container_name: &str, metric_name: &str) -> Result<Option<f64>> {
        match self {
...
pub trait MetricFetcher: Send + Sync {
    ...
    fn fetch_metric(&self, container_name: &str, metric_name: &str) -> impl std::future::Future<Output = Result<Option<f64>>> + Send;

    /// Fetch all metrics as raw Prometheus text (if applicable)
    fn fetch_raw(&self, container_name: &str) -> impl std::future::Future<Output = Result<String>> + Send;
}
...
pub fn format_metric(template: &str, value: f64) -> String {
    template
        ...
        .replace("{value_gb}", &format_number(value / (1024.0 * 1024.0 * 1024.0)))
...
mod tests {
    ...
    fn test_format_metric() {
        ...
        assert_eq!(format_metric("Progress: {value_pct}", 0.857), "Progress: 85.7%");
    }
